...
//            )
        ...
        desription = "Selected State was ${AttentionStates().list[selectedStateIndex.value].title} and the feedback was $desription"
        Spacer(modifier = Modifier.height(16.dp))
            Button(onClick = {
                val date = Logic().currentDateAndTime()
                val repo = FeedbackRepo(label)
                repo.insertFeedback(
                    FeedbackEntity(date, 0, desription, importance.toInt(), label)
                )
